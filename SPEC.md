...
**Lifetime level — never drops.** Separate from spendable XP, each kid has a **Level** derived from _lifetime XP ever earned_ = `sum(delta_cents where delta_cents > 0)`. Spending on quests never lowers it — it's pure progression/bragging rights, computed from the ledger (not stored), against a tunable level-threshold table (rising XP per level). Each level also has a fun **rank title** (Novice → Squire → Knight → Hero → Legend…, `~/utils/level.ts`), and crossing a level fires a **level-up celebration** (confetti + "You're a Knight!") on the kid dashboard. Personal quests carry an optional **emoji** icon the kid picks (migration 0017). The kid dashboard also shows a **daily streak** (🔥, `~/utils/streak.ts`), an **achievement-badge shelf** (`~/utils/achievements.ts`), and an **animated XP count-up + "+N" burst** on earnings — all derived from the ledger/chore data, no extra storage.

**Guild quests — everyone's XP auto-counts.** A `scope=guild` quest belongs to the household (no `kid_id`); only one is active at a time (enforced by a partial unique index — migration 0012). Every positive XP any member earns **after `started_at`** counts toward it automatically, and **personal balances are untouched** (the parent funds the family reward). Progress = `sum(delta_cents where delta_cents > 0 and created_at >= started_at)` across the household, capped at `target_xp`.
...
Because every user (parent, kid, co-parent) gets a real auth user, RLS (keyed on `auth.uid()`) applies uniformly. **The frontend talks to PostgREST directly for many mutations, so RLS — not the API — is the real boundary; policies must be tight enough to stand alone.** Two rules that follow from that:

- **Self-update is column-pinned.** A member editing their own `users` row can change display_name/avatar but **not** `role` or `household_id` (the `users_update_self` policy's `WITH CHECK` pins them), so a kid can't self-promote to parent or jump households.
- **Chore-instance writes are locked down.** RLS allows only **parents** to `UPDATE` `chore_instances` directly (that covers `approve`/`confirm`/`release`, run on the parent's JWT). Every **kid transition** (`claim`/`start`/`submit`/`mark-done`) goes through a `SECURITY DEFINER` RPC that locks the row, re-authorizes the caller, validates the current state, and writes the stored deadlines — so a kid can't PATCH an instance directly to inflate `value_cents_snapshot`, self-confirm a required chore, or steal a sibling's claim.

The other RLS-bypassing paths are minimal and sanctioned: creating/editing/deleting a kid runs with the **service role** (`POST/PATCH/DELETE /kids`, which re-checks household + role), the cron timer sweep runs as the service role, and the onboarding RPCs (`bootstrap_household`, `join_household`) are `SECURITY DEFINER` because the caller has no `users` row yet. Every `SECURITY DEFINER` function re-authorizes `auth.uid()` internally. Everything else goes through the caller's JWT.
