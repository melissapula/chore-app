...
export class ChoreInstancesService {
  ...
  async claim(user: AuthUser, id: string) {
    ...
    const chore = inst.chores as any;
    const household = inst.households as any;
  ...
  async start(user: AuthUser, id: string) {
    ...
    if (inst.state !== PaidState.CLAIMED) {
      throw new ConflictException(`Instance is ${inst.state}, not CLAIMED`);
    }
    if (inst.claimed_by !== user.id) {
      throw new ForbiddenException('Only the kid who claimed it can start it');
    }
  ...
  async submit(user: AuthUser, id: string) {
    ...
    if (inst.state !== PaidState.IN_PROGRESS) {
      throw new ConflictException(`Instance is ${inst.state}, not IN_PROGRESS`);
    }
    if (inst.claimed_by !== user.id) {
      throw new ForbiddenException('Only the kid working on it can submit it');
    }
    ...
    const { data: updated, error: updErr } = await db
      ...
      .maybeSingle();
    if (updErr) throw new BadRequestException(updErr.message);
    if (!updated) throw new ConflictException('Could not submit this chore');
    return updated;
  }
