-- ============================================================================
-- Chore App — 0002 Row-Level Security
-- Tenant isolation + role rules (SPEC §7). RLS is the real security boundary,
-- not the API. Every table is keyed on household_id.
--
-- Security model:
--   * Helper fns are SECURITY DEFINER so they read `users` WITHOUT triggering
--     RLS — this avoids infinite recursion in policies that reference the
--     current user's household/role.
--   * The cron timer sweep and parent-creates-kid onboarding run with the
--     Supabase SERVICE ROLE, which bypasses RLS. Everything a kid/parent does
...
--     the service enforces WHICH transition is legal.
-- ============================================================================
...
returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from users where id = auth.uid() and role = 'parent')
$$;

-- ---------------------------------------------------------------------------
-- Onboarding: create a household and the caller's parent row atomically.
-- Sidesteps the chicken-and-egg (no users row exists yet at signup).
...
begin
  ...
  insert into users (id, household_id, display_name, role, avatar_emoji)
...
create policy households_update on households
  for update using (id = app_household_id() and app_is_parent());
...
create policy users_update on users
  for update using (
    id = auth.uid()
    or (household_id = app_household_id() and app_is_parent())
  );
...
create policy chores_write_parent on chores
  for all using (household_id = app_household_id() and app_is_parent())
  with check (household_id = app_household_id() and app_is_parent());
...
create policy instances_write_parent on chore_instances
  for insert with check (household_id = app_household_id() and app_is_parent());

create policy instances_delete_parent on chore_instances
  for delete using (household_id = app_household_id() and app_is_parent());
...
create policy ledger_select on ledger_entries
  for select using (
    ...
    and (app_is_parent() or kid_id = auth.uid())
  );
...
create policy weekly_gates_update_parent on weekly_gates
  for update using (household_id = app_household_id() and app_is_parent())
  with check (household_id = app_household_id() and app_is_parent());
...
create policy goals_write_own on goals
  for all using (household_id = app_household_id() and kid_id = auth.uid())
  with check (household_id = app_household_id() and kid_id = auth.uid());
...
create policy notes_select on notes
  for select using (
    household_id = app_household_id()
    and (app_is_parent() or author_id = auth.uid() or recipient_id = auth.uid())
  );

create policy notes_insert on notes
  for insert with check (
    household_id = app_household_id() and author_id = auth.uid()
  );
